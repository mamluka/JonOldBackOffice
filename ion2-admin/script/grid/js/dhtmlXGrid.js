...
function dhtmlXGridObject(id){
 ...
 if(this._dload)
 this._dloadSize=Math.floor(parseInt(this.entBox.style.height)/80)+2;
 ...
 var distance = cell_obj.offsetTop+cell_obj.offsetHeight+80;
 ...
 if(ind<r.childNodes.length)
{
 ...
 hdrHTML+="<img  align='absmiddle' src='"+imUrl+"' hspace='2'>"
}
 hdrHTML+=label;
 hdrHTML+="</div>";
 ...
 if((!this._srClmn)||(this._srClmn[jj]))
{
 ...
 if(this._sAll)
 out+=(cvx._aimage?(" image='"+cvx._aimage+"'"):"")+">"+zxVal+"</cell>";
 else
 out+=">"+zxVal+"</cell>";
...
dhtmlXGridObject.prototype._hideShowColumn=function(ind,state){
 ...
 if(_isOpera||_isKHTML||(_isFF&&_FFrv<1.9)){
